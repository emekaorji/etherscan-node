...
export class LogsModule extends BaseModule {
  ...
  public async getLogs(params: Logs.LogsRequest): Promise<Logs.LogsResponse> {
    ...
    this.validateRequiredOr(params, ['address', 'startBlock']);
    this.validateAddressOr([params.address]);
    ...
    if (
      (params.topic0_1_opr && !validOperators.includes(params.topic0_1_opr)) ||
      (params.topic0_2_opr && !validOperators.includes(params.topic0_2_opr)) ||
      (params.topic1_2_opr && !validOperators.includes(params.topic1_2_opr))
    ) {
